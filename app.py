...
def generate_mission():
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "user", "content": f"User's vision: {vision}\nGenerate a mission based on this vision. Pull out a problem, a fact, a benefit, the tone should be like you're having a thought and you’re just writing what you’re thinking no emojis, no hashtags: Make it a coherent paragraph of text but keep it short, max 2 sentences. Make positive impact on reader in second sentence like achieving the mission is happening tomorrow and then after the 2 sentences create a mind map to visualize and accomplish the mission, making the user aware of the simplicity of achievement. At the end, come up with an idea for a simple application that solves the problem or benefit the user. (Format the answer using simple HTML tags, such as <h1>, <p>, or <br>, to enhance readability but dont say anything about this formating)(Remember to write in the language in which vision is written)(If the user puts a '?' at the end, simply answer their question)"
                }
            ]
        )
